...
ltol <- 0.0001
maxiter <- 10000
###Initialize W, M, V,L from mixtools
######
# if(p == 1){
#   init <- normalmixEM(X, k = J, epsilon = 1e-03, fast=TRUE)
# }else{
#   init <- mvnormalmixEM(X, k = J, epsilon = 1e-03, maxit = 10000, verb = FALSE )
# }
# W <- init$lambda
# M <- array(unlist(init$mu),dim = c(p,J))
# Sig <- array(unlist(init$sigma), dim = c(p, p, J))
# Ln <- init$loglik
# alpha_EM = (e-1)/f;
# Lp = -6.1110e+005;
# Lpo = 2*Lp;
